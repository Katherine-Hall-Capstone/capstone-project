...
function AppointmentsGrid({ appointments, role, onRead }) {
    ...
    const now = new Date()
    const upcomingAppointments = appointments.filter(appt => new Date(appt.endDateTime) >= now)
    const pastAppointments = appointments.filter(appt => new Date(appt.endDateTime) < now)
    ...
    return(
        <>
            <div className="mt-8 mx-6">
                <h2 className="text-2xl font-semibold italic mb-4 text-slate-600">Upcoming Appointments</h2>
                <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-center">
                    {upcomingAppointments.map(displayAppointmentCard)}
                </div>
            </div>

            <div className="mt-16 mx-6">
                <h2 className="text-2xl font-semibold italic mb-4 text-slate-600">Past Appointments</h2>
                <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-center">
                    {pastAppointments.map(displayAppointmentCard)}
                </div>
            </div>
